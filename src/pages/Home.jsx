...
const ICONS = { Buildings, ShieldCheck, Handshake, Briefcase, Users, Lock };
    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-navy-950">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence mode="sync">
          <motion.img key={currentSlide} src={heroImages[currentSlide]} alt="Kantor & Immerman" className="absolute inset-0 w-full h-[130%] object-cover object-center" initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.8, ease: 'easeInOut' }} loading="eager" />
        </AnimatePresence>
        <div className="absolute inset-0 bg-gradient-to-b from-navy-950/85 via-navy-950/55 to-navy-950/90 z-[1]" />
        <div className="absolute inset-0 bg-gradient-to-r from-navy-950/75 via-transparent to-navy-950/40 z-[1]" />
      </motion.div>

      {/* Slide indicators */}
      <div className="absolute right-5 sm:right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrentSlide(i)} className={`w-[3px] transition-all duration-500 ${i === currentSlide ? 'h-8 bg-gold-500' : 'h-4 bg-white/20 hover:bg-white/40'}`} aria-label={`Slide ${i + 1}`} />
        ))}
      </div>
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, delay: 0.3 }} className="w-12 h-[3px] bg-gold-500 mb-6 origin-left" />

        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="text-gold-400 text-xs sm:text-sm font-semibold uppercase tracking-[0.25em] mb-6 sm:mb-8">
          {hero.badge}
        </motion.p>

        <div className="overflow-hidden">
          {['JUSTICE', 'DELIVERED', 'WITH PRECISION.'].map((word, i) => (
            <motion.div key={word} initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ duration: 0.8, delay: 0.5 + i * 0.12, ease: [0.16, 1, 0.3, 1] }}>
              <h1 className={`font-heading font-bold leading-[0.88] tracking-tight ${word === 'WITH PRECISION.' ? 'text-gold-500' : 'text-white'}`} style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>
                {word}
              </h1>
            </motion.div>
          ))}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.3 }} className="flex flex-wrap gap-4 mt-8 sm:mt-10">
          <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-gold-500 text-white px-7 py-3.5 sm:py-4 font-heading text-sm sm:text-base uppercase tracking-wider font-semibold transition-all duration-300 hover:bg-gold-600 hover:shadow-xl hover:shadow-gold-500/20">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.6 }} className="flex flex-wrap items-center gap-6 mt-10 sm:mt-14">
          <div className="flex items-center gap-2 text-white/40 text-xs sm:text-sm"><CheckCircle size={16} weight="fill" className="text-gold-500" /><span>Law Society of Zimbabwe</span></div>
          <div className="flex items-center gap-2 text-white/40 text-xs sm:text-sm"><CheckCircle size={16} weight="fill" className="text-gold-500" /><span>Established 1962</span></div>
          <div className="flex items-center gap-2 text-white/40 text-xs sm:text-sm"><CheckCircle size={16} weight="fill" className="text-gold-500" /><span>5,000+ Cases Won</span></div>
        </motion.div>
      </motion.div>
      ...
      <div className="hidden lg:flex absolute right-8 top-1/2 -translate-y-1/2 z-20">
        <span className="text-white/15 text-[10px] uppercase tracking-[0.4em] font-heading" style={{ writingMode: 'vertical-rl' }}>Kantor &amp; Immerman &mdash; Harare, Zimbabwe</span>
      </div>
    </section>
  );
}
...
function MarqueeTicker() {
  const items = ['CORPORATE LAW', 'LITIGATION', 'PROPERTY LAW', 'LABOUR LAW', 'FAMILY LAW', 'COMMERCIAL CONTRACTS', 'DISPUTE RESOLUTION', 'CONVEYANCING'];
      ...
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-4 sm:gap-6 mx-4 sm:mx-6">
            <span className="text-gold-500 font-heading text-lg sm:text-2xl font-bold uppercase tracking-wider">{item}</span>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }}>
            ...
            <h2 className="font-heading font-bold text-white leading-[0.92] mb-8" style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>
              SIX DECADES<br />OF LEGAL<br /><span className="text-gold-500">AUTHORITY.</span>
            </h2>
            <p className="text-white/50 text-sm sm:text-base leading-relaxed mb-6 max-w-lg">
              Kantor &amp; Immerman was established in 1962, during a pivotal era in Zimbabwe's history. From our earliest days,
              we committed to providing legal services of the highest calibre &mdash; services that stand up not just in court, but in the annals of legal precedent.
            </p>
            <p className="text-white/40 text-sm leading-relaxed max-w-lg">
              Today, with over 40 legal professionals across six practice areas, we continue to set the standard for legal
              excellence in Zimbabwe. Our partners are recognised authorities in corporate law, property law, and litigation.
            </p>
            <div className="w-full h-px bg-white/10 my-8" />
            <div className="flex gap-8 sm:gap-12">
              <div><div className="text-gold-500 font-heading text-3xl sm:text-4xl font-bold">1962</div><div className="text-white/40 text-xs uppercase tracking-wider mt-1">Founded</div></div>
              <div><div className="text-gold-500 font-heading text-3xl sm:text-4xl font-bold">5K+</div><div className="text-white/40 text-xs uppercase tracking-wider mt-1">Cases Won</div></div>
              <div><div className="text-gold-500 font-heading text-3xl sm:text-4xl font-bold">40+</div><div className="text-white/40 text-xs uppercase tracking-wider mt-1">Professionals</div></div>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }} className="relative">
            <div className="relative">
              <div className="overflow-hidden"><img src="https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=800" alt="Legal library" className="w-full aspect-[4/5] object-cover object-center" loading="lazy" /></div>
              <div className="absolute -bottom-8 -left-6 sm:-left-10 w-[45%] overflow-hidden border-4 border-navy-950 shadow-2xl">
                <img src="https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=600" alt="Legal documents" className="w-full aspect-square object-cover object-center" loading="lazy" />
              </div>
              <div className="absolute -top-4 -right-4 sm:-right-6 bg-gold-500 text-white p-4 sm:p-6 shadow-2xl">
                <div className="text-center"><div className="font-heading text-2xl sm:text-3xl font-bold leading-none">60+</div><div className="text-[10px] sm:text-xs uppercase tracking-wider font-semibold mt-1 text-white/80">Years of<br />Excellence</div></div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="mb-12 sm:mb-16">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <h2 className="font-heading font-bold text-white leading-[0.92]" style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>PRACTICE <span className="text-gold-500">AREAS</span></h2>
            <Link to="/services" className="group text-white/40 text-sm uppercase tracking-wider font-heading flex items-center gap-2 hover:text-gold-500 transition-colors">All Practice Areas <ArrowRight size={14} className="transition-transform group-hover:translate-x-1" /></Link>
          </div>
        </motion.div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {servicesPreview.map((service, i) => {
            const Icon = ICONS[service.iconName] || Buildings;
            return (
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 * i }}>
                <Link to={`/services/${siteData.services?.items?.[i]?.slug || '#'}`} className="group relative block overflow-hidden aspect-[4/5] sm:aspect-[3/4]">
                  ...
                  <div className="absolute top-4 right-4 z-10"><span className="text-gold-500/30 font-heading text-5xl sm:text-6xl font-bold leading-none">{String(i + 1).padStart(2, '0')}</span></div>
                  <div className="absolute top-4 left-4 z-10 w-10 h-10 bg-gold-500/20 backdrop-blur-sm flex items-center justify-center"><Icon size={20} className="text-gold-400" /></div>
  ...
  return (
    <section ref={ref} className="relative bg-navy-950 overflow-hidden">
      <div className="absolute inset-0">
        <img src="https://images.unsplash.com/photo-1521737852567-6949f3f9f2b5?w=1920" alt="" className="w-full h-full object-cover object-center" loading="lazy" />
          ...
          {siteData.stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.1 }} className="text-center">
              <div className="font-heading font-bold text-white leading-none" style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>
                <AnimatedCounter target={stat.number} duration={2} />{stat.number.includes('+') && <span className="text-gold-500">+</span>}
              </div>
...
function HorizontalGallery() {
  ...
  const galleryImages = [
    { src: 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=800', caption: 'Supreme Court Precedent' },
    { src: 'https://images.unsplash.com/photo-1521737852567-6949f3f9f2b5?w=800', caption: 'Property Dispute Won' },
    { src: 'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=800', caption: 'Labour Class Action' },
    { src: 'https://images.unsplash.com/photo-1556761175-5973dc0f32e7?w=800', caption: 'Corporate Merger' },
    { src: 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=800&q=80', caption: 'Estate Resolution' },
    { src: 'https://images.unsplash.com/photo-1521737852567-6949f3f9f2b5?w=800&q=80', caption: 'Lease Portfolio' },
    { src: 'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=800&q=80', caption: 'Regulatory Victory' },
    { src: 'https://images.unsplash.com/photo-1556761175-5973dc0f32e7?w=800&q=80', caption: 'Cross-Border Deal' },
  ];
    ...
    <section ref={containerRef} className="bg-navy-950 py-20 sm:py-28 lg:py-32 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-10 sm:mb-14">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            <div className="w-10 h-[3px] bg-gold-500 mb-6" />
            <h2 className="font-heading font-bold text-white leading-[0.92]" style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>LANDMARK <span className="text-gold-500">CASES</span></h2>
          </div>
          <Link to="/projects" className="group text-white/40 text-sm uppercase tracking-wider font-heading flex items-center gap-2 hover:text-gold-500 transition-colors">View All <ArrowRight size={14} className="transition-transform group-hover:translate-x-1" /></Link>
        </motion.div>
      </div>
          ...
          {galleryImages.map((img, i) => (
            <div key={i} className="group relative flex-shrink-0 w-[280px] sm:w-[320px] lg:w-[380px] overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                <span className="text-gold-500 font-heading text-xs uppercase tracking-wider font-semibold">{String(i + 1).padStart(2, '0')}</span>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }} className="relative">
            ...
            <div className="absolute bottom-6 left-6 bg-navy-950/90 backdrop-blur-sm p-5">
              <div className="text-gold-500 font-heading text-4xl font-bold">{whyChooseUs.experienceYears}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }}>
            ...
            <h2 className="font-heading font-bold text-white leading-[0.92] mb-10" style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>
              {whyChooseUs.titleParts.map((p, i) => p.highlight ? <span key={i} className="text-gold-500">{p.text}</span> : <span key={i}>{p.text}</span>)}
            </h2>
            <div className="space-y-6">
              {whyChooseUs.points.map((point, i) => (
                <motion.div key={point.title} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.3 + i * 0.1 }} className="flex gap-4">
                  <div className="w-10 h-10 bg-gold-500/10 flex items-center justify-center shrink-0 mt-1"><CheckCircle size={20} weight="fill" className="text-gold-500" /></div>
                  <div>
                    <h3 className="font-heading text-white text-base sm:text-lg font-bold uppercase tracking-wide mb-1">{point.title}</h3>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }}>
              <div className="w-16 h-16 mx-auto mb-6 overflow-hidden rounded-full border-2 border-gold-500/30">
                <img src={avatars[active % avatars.length]} alt={t.name} className="w-full h-full object-cover" />
              </div>
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex items-center gap-0.5 mt-1">{[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-gold-500" />)}</div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-10">
            ...
            <div className="flex gap-2">{homeTestimonials.map((_, i) => (<button key={i} onClick={() => setActive(i)} className={`h-[3px] transition-all duration-300 ${i === active ? 'w-8 bg-gold-500' : 'w-3 bg-white/15 hover:bg-white/30'}`} aria-label={`Testimonial ${i + 1}`} />))}</div>
    ...
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-44 overflow-hidden">
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8 }}>
          <h2 className="font-heading font-bold text-white leading-[0.88] mb-8" style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>JUSTICE DOES<br />NOT <span className="text-gold-500">WAIT.</span></h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-gold-500 text-white px-8 py-4 font-heading text-sm sm:text-base uppercase tracking-wider font-semibold transition-all duration-300 hover:bg-gold-600 hover:shadow-xl hover:shadow-gold-500/20">
